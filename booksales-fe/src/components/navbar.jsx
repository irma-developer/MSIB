...
import { getUser, logout } from "../_service/auth";
import useDecodedToken from "../hooks/useDecodedToken";

export default function Navbar() {
  const nav = useNavigate();
  const { token, expired, decoded } = useDecodedToken();
  const user = getUser();
  const isAuthed = !!token && !expired;
  const displayName = user?.name || decoded?.name || decoded?.sub || "User";
      ...
      <header>
        <nav className="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
          <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            {/* Button kanan (login / register) */}
            <div className="flex items-center lg:order-2">
              {isAuthed ? (
                <>
                  <span className="mr-3 text-gray-700 dark:text-gray-200 text-sm hidden sm:inline">
                    {displayName}
                  </span>
                  <Link
                    to="/admin"
                    className="text-gray-800 dark:text-white hover:bg-gray-50 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2 mr-2 dark:hover:bg-gray-700"
                  >
                    Dashboard
                  </Link>
                  <button
